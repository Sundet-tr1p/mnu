...
export default async function ProfilePage() {
  ...
  if (!profile) return <div className="p-8">Профиль не найден</div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        ...
        <div className="mb-6 grid grid-cols-2 gap-4">
          <div className="rounded-xl bg-gray-50 p-4 text-center">
            ...
            <div className="text-sm text-gray-500">Посты</div>
          </div>
          <div className="rounded-xl bg-gray-50 p-4 text-center">
        ...
        <div className="space-y-3 text-sm">
          {profile.school && (
            <div className="flex gap-2">
              <span className="w-28 text-gray-400">Школа:</span>
              <span className="text-gray-900">{profile.school}</span>
            </div>
          )}
          {profile.specialty && (
            <div className="flex gap-2">
              <span className="w-28 text-gray-400">Специальность:</span>
              <span className="text-gray-900">{profile.specialty}</span>
            </div>
          )}
          {profile.bio && (
            <div className="flex gap-2">
              <span className="w-28 text-gray-400">О себе:</span>
              <span className="text-gray-900">{profile.bio}</span>
            </div>
          )}
          <div className="flex gap-2">
            <span className="w-28 text-gray-400">Регистрация:</span>
